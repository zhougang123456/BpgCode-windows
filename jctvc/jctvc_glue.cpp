...
static HEVCEncoderContext *jctvc_open(const HEVCEncodeParams *params)
{
    ...
    strcpy(buf, "/tmp/");
#endif
    snprintf(s->infilename, sizeof(s->infilename), "%sout%d-%d.yuv", buf, 100, tmp_idx);
    snprintf(s->outfilename, sizeof(s->outfilename), "%sout%d-%d.bin", buf,100, tmp_idx);
    tmp_idx++;
